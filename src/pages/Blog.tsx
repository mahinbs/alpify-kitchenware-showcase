  ...
  return <div className="min-h-screen bg-gradient-to-br from-background to-muted/20">
      ...
      <section className="pt-32 pb-20 relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <motion.div className="flex flex-col md:flex-row md:items-center md:justify-between mb-12 space-y-4 md:space-y-0" initial={{
          opacity: 0,
          y: 30
        }} animate={{
          opacity: 1,
          ...
          delay: 0.2
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Search articles..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-3 border border-border rounded-lg bg-background text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent w-full md:w-80" />
              </div>

              {/* Category Filter */}
              <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="px-4 py-3 border border-border rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
      ...
      <section className="py-20 bg-background relative">
        <div className="max-w-7xl mx-auto px-6">
          ...
          opacity: 1
              ...
              <p className="text-muted-foreground">Loading articles...</p>
               ...
               {filteredBlogs.map((blog, index) => <article key={blog.id} className="blog-item bg-card border border-border rounded-2xl overflow-hidden shadow-elegant hover:shadow-glow transition-all duration-500 group">
                  ...
                  <div className="p-6">
                    ...
                    {/* Tags */}
                    {blog.tags && blog.tags.length > 0 && <div className="flex items-center space-x-2 mb-6">
                        <Tag className="w-4 h-4 text-muted-foreground" />
                        <div className="flex flex-wrap gap-1">
                          {blog.tags.slice(0, 3).map((tag, tagIndex) => <span key={tagIndex} className="px-2 py-1 bg-muted rounded-full text-xs">
                              {tag}
                            </span>)}
                          {blog.tags.length > 3 && <span className="px-2 py-1 bg-muted rounded-full text-xs">
                              +{blog.tags.length - 3}
                            </span>}
                        </div>
                      </div>}
                    ...
                    <Link to={`/blog/${blog.id}`}>
                      <motion.button className="w-full px-6 py-3 bg-gradient-to-r from-primary to-vibrant-orange text-white rounded-lg font-semibold hover:shadow-glow transition-all duration-300" whileHover={{
                  scale: 1.02
                }} whileTap={{
                  scale: 0.98
                }}>
             ...
             </div> : blogs.length === 0 ? <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} className="text-center py-20">
            ...
            </motion.div> : filteredBlogs.length === 0 ? <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} className="text-center py-20">
          ...
          }} className="px-6 py-3 bg-gradient-to-r from-primary to-vibrant-orange text-white rounded-lg font-semibold hover:shadow-glow transition-all duration-300" whileHover={{
            scale: 1.05
          }} whileTap={{
            scale: 0.95
          }}>
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          duration: 0.8
            ...
            <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
              <input type="email" placeholder="Enter your email address" className="flex-1 px-6 py-3 border border-border rounded-lg bg-background text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
              <motion.button className="px-8 py-3 bg-gradient-to-r from-primary to-vibrant-orange text-white rounded-lg font-semibold hover:shadow-glow transition-all duration-300" whileHover={{
              scale: 1.05
            }} whileTap={{
              scale: 0.95
            }}>
